    ...
    public class GL21Renderer : IRenderer
    {
        ...
        public uint CreateTexture(uint width, uint height)
        {
            ...
            try
            {
                ...
                (
                    id, 
                    TextureParameterName.TextureMagFilter, new int[]
                    { (int)TextureMagFilter.Linear/*.Nearest*/ }
                );
            }
            catch (Exception ex) 
            {
                Debug.WriteLine("[ex] GL.TextureParameterI bug: " + ex.Message);
            }
            ...
            { 
                ...
                (
                    id, 
                    TextureParameterName.TextureMinFilter, new int[] 
                    { (int)TextureMinFilter.Linear/*.Nearest*/ }
                );
            }
            catch (Exception ex)
            {
        ...
        public void UpdateTexture(uint id, Sprite spr)
        {
            GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba,
                (int)spr.Width, (int)spr.Height, 0, PixelFormat.Rgba, PixelType.UnsignedInt8888,
                spr.ColorData);
        }
